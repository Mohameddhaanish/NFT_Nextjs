    ...
    <div className="flexBetweenStart my-5">
      <div className="flex-1 flexStartCenter">
        <div className="relative w-28 h-28">
          <Image src={nft.tokenURI || images[`nft${nft.i}`]} layout="fill" objectFit="cover" />
        </div>
    ...
    <div className="relative flex justify-center md:flex-col min-h-screen">
      <div className="relative flex-1 flexCenter sm:px-4 p-12 border-r md:border-r-0 md:border-b dark:border-nft-black-1 border-nft-gray-1">
        <div className="relative w-557 minmd:w-2/3 minmd:h-2/3 sm:w-full sm:h-300 h-557 ">
          <Image src={nft.tokenURI || images[`nft${nft.i}`]} objectFit="cover" className=" rounded-xl shadow-lg" layout="fill" />
        </div>
      </div>
            ...
            : currentAccount === nft.owner.toLowerCase()
              ? (
                <Button
                  ...
                  handleClick={() => router.push(`/resell-nft?id=${nft.tokenId}&tokenURI=${nft.tokenURI}&prices=${nft.price}`)}
                />
              )
              : (
                <Button
        ...
        body={(
          <div className="flexCenter flex-col text-center" onClick={() => setSuccessModal(false)}>
            <div className="relative w-52 h-52">
              <Image src={nft.tokenURI || images[`nft${nft.i}`]} objectFit="cover" layout="fill" />
            </div>
